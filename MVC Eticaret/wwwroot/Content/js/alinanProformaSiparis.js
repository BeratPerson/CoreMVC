...
function Kaydet() {
    ...
    var Satici_Kodu = document.getElementById("Satici_Adi").value;
    if (Musteri_Kodu == "") {
        ...
        HataYazdir('Bir Satıcı Seçiniz!');
        sorun++;
    }
    else if (list.length == 0) {
        HataYazdir('Bir Proforma Sipariş Detayı Ekleyiniz!');
        sorun++;
    }
    ...
    if (sorun == 0) {
        ...
        //    }
        //});
        var valdata = $("#proformaSiparisform").serialize();
        $.ajax({
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            type: 'POST',
            url: '/ProformaSiparis/YeniAlinanProformaSiparisOlustur',
            data: JSON.stringify(Proforma_Siparis_Ekle),
                ...
                if (data.status === 'False') {
                    ...
                    if (valdata == "") {
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            type: 'POST',
                            url: '/ProformaSiparis/YeniAlinanProformaSiparisOlusturUser',
                            data: valdata,
                            success: function (data) {
                                if (data.status === 'False') {
                                    $('#BasarisizKayit').show();
                                    ...
                                    $.ajax({
                                        type: 'POST',
                                        url: '/ProformaSiparis/YeniAlinanProformaSiparisOlusturFile',
                                        dataType: "json",
                                        contentType: false,
                                        processData: false,
                                        data: fileData,
...
function Satici_Kodu_OnChange() {
    var tip = document.getElementById("Satici_Kodu").value;
    var Id = $("#Satici_Kodu option:selected").html();
    s = '<option value="' + Id + '">' + tip + '</option>';
    $("#Satici_Adi").html(s);
}
function Satici_Adi_OnChange() {
    var tip = document.getElementById("Satici_Adi").value;
    var Id = $("#Satici_Adi option:selected").html();
    s = '<option value="' + Id + '">' + tip + '</option>';
    $("#Satici_Kodu").html(s);
}
function Hareket_Tipi_Kodu_OnChange() {
    ...
    if (tip == "0" || tip == "1" || tip == "2" || tip == "3") {
        $.ajax({
            type: "GET",
            url: "/ProformaSiparis/getirStokKoduCinsineGore?tip=" + tip,
            data: "{}",
            success: function (data) {
                $("#Stok_Kod").html('');
...
function htmlDoldur() {
    ...
    if (list.length != 0) {
        ...
        html += '</tbody>';
        $('#proformaSiparisTable').html(html);
    }
    else {
        $('#proformaSiparisTable').html(html);
    }
